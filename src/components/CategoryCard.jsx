  ...
  if (compact && accent) {
    ...
    return (
      <button
        type="button"
        onClick={onClick}
        className={`w-full rounded-2xl border p-4 text-left shadow-sm transition active:scale-[0.99] ${accentClass}`}
      >
        <div className="flex items-start justify-between gap-3">
          ...
          {totalCount != null && totalLabel && (
            <div className="shrink-0 rounded-xl bg-white/15 px-2.5 py-1.5 text-center">
              <p className="text-lg font-bold leading-none">{totalCount}</p>
              <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-wide text-white/85">
                {totalLabel}
              </p>
            </div>
          )}
